...
case "$OS" in
  ...
  wasi)
    ...
    # Stub threading headers (std::mutex, std::condition_variable, std::thread)
    # for single-threaded WASI — shadows the missing sysroot headers.
    ...
    cp "$PATCHES/wasi/include/thread" "build/config/wasi/include/thread"
    ;;
